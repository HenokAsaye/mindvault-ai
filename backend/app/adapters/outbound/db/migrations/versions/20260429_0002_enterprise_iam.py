...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_refresh_tokens_org_id", "refresh_tokens", ["org_id"], unique=False
    )
    ...
    op.create_index(
        "ix_audit_logs_target_user_id", "audit_logs", ["target_user_id"], unique=False
    )

    op.create_index(
        "ix_membership_org_status",
